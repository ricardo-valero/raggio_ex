...
`Raggio.Syntax` provides a macro-free toolkit for building, wrapping, traversing, and transforming syntax trees of schema, field, and type nodes. It offers composable node builders, a tree wrapper with metadata, traversal and query combinators, structure-preserving transformations, and a node protocol so traversal and transformation operate uniformly over node structs.
