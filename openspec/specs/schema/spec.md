...
`Raggio.Schema` provides a macro-free, function-based schema system for defining and validating data. It offers immutable constructors for primitive and composite types, a small set of core constraints, and a "parse, don't validate" model that turns input into well-typed domain data while reporting structured, path-aware errors. It also supports validation modes, coercion, and bidirectional transforms for round-trip data handling.
